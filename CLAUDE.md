...
- A user's site is viewed at `/site/{slug}/` on any host, at `/` on `{slug}.MAIN_HOST` (in production `milton.kidsthatteach.org`, reached through the Cloudflare Worker in `deploy/cloudflare-worker/`; locally simulate it with `X-Forwarded-Host` + `X-Site-Proxy-Key` headers), or at `/` on its custom domain (`sites.domain`).
...
- Deployment (DreamHost VPS with one shared directory for every hostname, Cloudflare DNS + Worker for `*.kidsthatteach.org`, Cloudflare R2 bucket + CORS; migrating videos off DreamObjects): `docs/deployment.md`.
