...
- Copy `www/config.local.php.example` to `www/config.local.php` and fill in DB credentials (DreamObjects keys are optional locally; video upload is disabled without them).
...
- Deployment (DreamHost VPS, three hostnames on one directory, DreamObjects bucket + CORS): `docs/deployment.md`.
